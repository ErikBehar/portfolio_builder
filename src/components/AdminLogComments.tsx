...
export function AdminLogComments({
  ...
  async function handleSave(commentId: string) {
    ...
    const response = await fetch(
      `/api/log/${logEntryId}/comments/${commentId}`,
      {
        method: "PUT",
  ...
  return (
    <section className="mt-12 border-t border-border pt-10">
      ...
      <p className="mt-1 text-sm text-muted">
        Edit or remove comments left on this log entry.
      </p>

      {error && <p className="mt-4 text-sm text-red-400">{error}</p>}

      {comments.length === 0 ? (
        <p className="mt-6 text-sm text-muted">No comments on this entry yet.</p>
      ) : (
